...
(function() {
    ...
    var vikisubs = {
        ...
        css: `
                ...
                a.subtitle-link-tamper > span.convert{float: right;}
            `,
        subtitles: [],
    ...
    converter = {
        ...
        title: function(lang) {
            ep = video.number;
            title = video.container.titles.en + '.1x';
            if (ep < 10)
                title += '0';
            title += ep;
            title += '.' + lang;
        ...
        init: function() {
            converter.links = $(converter.links);

            if (converter.links.length > 0) {
                converter.form = $(converter.form);
                $('body').append(converter.iframe);
                converter.links.each(function() {
                    $(this).append('  <span class="convert">(SRT)</span>');
                    $(this).find('span.convert').on('click', function(e) {
                        e.preventDefault();
                        link = $(this).parent('a').attr('href');
                        lang = $(this).parent('a').attr('data-lang');
                        var title = converter.title(lang);
                        $.get(link, function(data) {
                            if (data.length && data.indexOf('WEBVTT') !== -1) {
                                //encoded = btoa(unescape(encodeURIComponent( data )));
                                src = converter.src + title + ".srt";
                                form = converter.form.clone();
                                form.attr('action', src).find('textarea').text(data);
                                $('body').append(form);
                                form.submit().remove();
                            }
                        });
